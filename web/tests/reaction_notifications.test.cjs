...
const message_viewport = mock_esm("../src/message_viewport", {
    // Measuring the real viewport needs a rendered feed, so tests say
    // directly which messages are on screen. Nothing is, by default.
    visible_messages: () => [],
});
...
function raw_message(id) {
    return {
        ...
        submessages: [],
        timestamp: 1000,
        // A message the current user sent themselves is already read.
        flags: ["read"],
        type: "stream",
        stream_id: general.stream_id,
        topic: "whatever",
...
test("reactions that cannot notify are dropped without a fetch", ({override}) => {
    ...
    // With only the audible setting on, a sound can still play without the
    // browser's desktop-notification permission, so the message is worth
    // fetching even though no desktop notification can result.
    override(user_settings, "enable_reaction_audible_notifications", true);
    ...
    // ...except when the notification sound is "none", which leaves the
    // audible setting with nothing to play, so again nothing could result.
    override(user_settings, "notification_sound", "none");
    get_called = false;
    received_reaction(event);
    assert.ok(!get_called);

    // Restore a playable sound for the desktop-only cases below.
...
test("reaction notification titles", ({override}) => {
    ...
    // In particular, the unrenderable emoji_code is not shown raw.
    assert.ok(!notices.last_title.includes("not-a-codepoint"));

    // A user on the text emojiset has opted out of emoji images, so even a
    // valid Unicode reaction is credited as `:emoji_name:` rather than as
    // the glyph.
    override(user_settings, "emojiset", "text");
    const text_emojiset_message = my_channel_message(3320);
    react(text_emojiset_message, alice.user_id);
    assert.equal(notices.last_title, "translated: Alice reacted with :tada:");
});
...
test("removing a reaction updates or dismisses its notification", () => {
    ...
    // The notification keeps the title it already had, which still credits
    // the reaction just removed: rewriting it means replacing the
    // notification, which would re-pop it for something the user no longer
    // needs to see.
    assert.equal(notices.last_title, "translated: Bob and Alice reacted with \u{1F44D}, \u{1F389}");
...
test("a removal dismisses its notification even when notifications are off", ({override}) => {
    // Removals are handled whatever the settings currently say. A user who
    // turns reaction notifications off while one is on screen should still
    // have it come down when the reaction behind it is retracted, rather
    // than being left with a notification nothing can dismiss.
    const notices = stub_notification_api();
    stub_message_content_parsing();
    const message = my_channel_message(3500);
    react(message, alice.user_id);
    assert.ok(desktop_notifications.get_notifications().has(message.id.toString()));
...
test("reactions notify unless the user is watching the message", ({override}) => {
    ...
    const message_on_screen = () => [message];
    const scrolled_past_message = () => [];
    ...
    // the reaction notifies, even with the message in the current list. The
    // viewport is not consulted at all in this case.
    ...
    // The message is rendered in the current list, but the user has scrolled
    // away from it, so they cannot see the reaction arrive and we notify.
    override(message_lists, "current", list_showing_message);
    override(message_viewport, "visible_messages", scrolled_past_message);
    assert.equal(reactions_shown(), 1);

    // Focused, watching the feed, and the message is on screen: the reaction
    // appears on a message in front of the user, so we stay quiet.
    override(message_viewport, "visible_messages", message_on_screen);
