...
// dropped from whichever batches are holding it, so that we do not go on
// to notify about a reaction that no longer exists. Each batch owns its
// own set, so that one can neither cancel nor revive another's pending
// notifications when the same reaction is added, retracted, and added
// again while a fetch is outstanding. Batches are keyed by an id rather
// than by the set itself, so that a batch can be discarded by name.
const pending_reaction_notifications = new Map<number, Set<string>>();
...
function discard_pending_reaction(event: ReactionEvent): void {
    ...
    for (const pending_keys of pending_reaction_notifications.values()) {
...
function message_is_in_focused_view(message: Message): boolean {
    // Whether the user is currently looking at this message. Reactions
    // they can already see don't need a notification. Being focused on
    // another conversation should still notify, as with message
    // notifications. viewport_is_visible_and_focused handles background
    // tabs, overlays/modals, and views that replace the message feed.
    if (!unread_ops.viewport_is_visible_and_focused()) {
        return false;
    }

    // Being rendered in the current message list is not enough, since the
    // user can have scrolled far away from the message, so we ask the
    // viewport which messages are actually on screen. The message list is
    // still checked first, because visible_messages requires one and
    // measures the DOM, which we would rather not do per reaction event.
    if (message_lists.current?.get(message.id) === undefined) {
        return false;
    }

    // A partially visible message counts as seen: a message taller than
    // the viewport is never fully visible, and the user is plainly
    // looking at it.
    return message_viewport
        .visible_messages(false)
        .some((visible_message) => visible_message.id === message.id);
}
...
function fetch_messages_for_reactions(
    ...
    void channel.get({
        ...
        success(raw_data) {
            pending_reaction_notifications.delete(batch_id);
...
export function received_reactions(events: ReactionNotificationEvent[]): void {
    ...
    pending_reaction_notifications.set(batch_id, pending_keys);
    ...
    try {
        ...
        for (const event of events) {
            ...
            // the position of the add that was retracted. The title credits
            // reactors in arrival order, so the newest add must sort last.
        ...
        if (reactions_by_message_id.size > 0) {
            fetch_messages_for_reactions(reactions_by_message_id, batch_id, pending_keys);
